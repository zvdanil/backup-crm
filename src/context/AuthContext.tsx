...
async function fetchOrCreateProfile(user: User): Promise<UserProfile | null> {
  ...
  if (existingError) {
    logAuth("fetchOrCreateProfile:error:select", {
  ...
  // Профиль не найден - создаем новый с ролью 'newregistration' (требует активации админом)
  const role: UserRole = "newregistration";
  const fullName =
    user.user_metadata?.full_name || user.user_metadata?.parent_name || null;
  const parentName = user.user_metadata?.parent_name || null;
  const childName = user.user_metadata?.child_name || null;
  logAuth("fetchOrCreateProfile:create", {
    userId: user.id,
    role,
    fullName,
    parentName,
  ...
  if (createError) {
    ...
    if (
      ...
      if (retryError) {
        logAuth("fetchOrCreateProfile:error:retry", {
    ...
    logAuth("fetchOrCreateProfile:error:create", {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const profileRetryTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
    ...
    async (event: string, newSession: Session | null) => {
      ...
      if (newSession?.user) {
        ...
        if (lastProfileUserIdRef.current === userId) {
          logAuth("authStateChange:profile-skip", {
            userId,
        ...
        const isNewRegistration =
          event === "SIGNED_IN" && !lastProfileUserIdRef.current;
        ...
        try {
          ...
          logAuth("authStateChange:profile-error", {
            ...
            const retryLoad = async () => {
              ...
              if (
                ...
                try {
                  ...
                  lastProfileUserIdRef.current = currentUserId;
                  logAuth("authStateChange:profile-retry-success", {
                    userId: currentUserId,
                    retryCount,
                  });
                  setIsLoading(false);
                } catch (retryError: any) {
                  logAuth("authStateChange:profile-retry-error", {
                    message: retryError?.message,
                    retryCount,
                  });
  ...
  useEffect(() => {
    if (!supabaseUrl || !supabaseKey) {
      ...
      toast({
        ...
        description:
          "Не задано VITE_SUPABASE_URL або VITE_SUPABASE_PUBLISHABLE_KEY.",
        variant: "destructive",
      });
      setSession(null);
      setUser(null);
    ...
    supabase.auth
        ...
        if (session?.user) {
          ...
          if (cached) {
            ...
            handleAuthChange("INITIAL_SESSION", session).catch((error) => {
              console.error(
                "[Auth] Error in handleAuthChange during initial session",
                error,
              );
      ...
      .catch((error) => {
        console.error(
          "[Auth] Error getting initial session (promise rejection)",
          error,
        );
        setIsLoading(false);
      });
    ...
    const { data: subscription } = supabase.auth.onAuthStateChange(
      async (event, newSession) => {
        ...
        if (event === "TOKEN_REFRESHED") {
          logAuth("authStateChange:token-refreshed", {
