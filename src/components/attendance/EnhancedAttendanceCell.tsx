...
const baseStatuses: BaseAttendanceStatus[] = [
  "present",
  "sick",
  "absent",
  "vacation",
...
export function EnhancedAttendanceCell({
  ...
  const statusColor = displayStatus
    ? getAttendanceColor(displayStatus, customStatuses)
    : "";
  const isBaseStatus = displayStatus
    ? isBaseAttendanceStatus(displayStatus)
    : false;
  ...
  const getPriceHint = (statusKey: AttendanceStatus | string): string => {
    ...
    if (customPrice !== null && customPrice !== undefined) {
      ...
      const finalPrice =
        Math.round(customPrice * discountMultiplier * 100) / 100;
    ...
    let rule = billingRulesForDate[statusKey as keyof BillingRules] as
      | BillingRule
      | undefined;
    ...
    if (!rule && billingRulesForDate.custom_statuses) {
      const customStatus = billingRulesForDate.custom_statuses.find(
        (cs: CustomAttendanceStatus) =>
          cs.id === statusKey && cs.is_active !== false,
      );
      if (customStatus) {
        rule = {
    ...
    switch (rule.type) {
      case "fixed": {
        ...
        const discounted =
          Math.round(rule.rate * discountMultiplier * 100) / 100;
      ...
      case "subscription": {
        ...
        const dailyPrice =
          workingDays > 0
            ? Math.round((rule.rate / workingDays) * 100) / 100
            : 0;
        const discounted =
          Math.round(dailyPrice * discountMultiplier * 100) / 100;
  ...
  const getCalculatedValueForStatus = (
  ...
  if (!isBaseStatus && statusColor) {
    ...
    cellStyle.color =
      getColorBrightness(statusColor) < 128 ? "#FFFFFF" : "#000000";
  }
      ...
      <PopoverContent className="w-64 p-3" align="center">
        <div className="space-y-3">
              ...
              return (
                  ...
                  style={
                    !isBase && status === s && statusColor
                      ? {
                          ...
                          color:
                            getColorBrightness(statusColor) < 128
                              ? "#FFFFFF"
                              : "#000000",
                ...
                >
                  <span className="font-semibold">
                    {getAttendanceLabel(s, customStatuses)}
                  </span>
          ...
          {customStatuses.filter(
            (cs: CustomAttendanceStatus) => cs.is_active !== false,
          ).length > 0 && (
                  ...
                  .map((cs: CustomAttendanceStatus) => {
                    ...
                    return (
                      <Button
                        ...
                        style={{
                          backgroundColor:
                            status === cs.id ? cs.color : undefined,
                          color:
                            status === cs.id && brightness < 128
                              ? "#FFFFFF"
                              : undefined,
                          borderColor:
                            status === cs.id ? "transparent" : undefined,
                        }}
                      >
