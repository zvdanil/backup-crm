-- ============================================
-- Применить версию 4: distribute_advance_payment с обработкой возвратов
-- Логика:
-- ШАГ 1: Находим активности с положительным балансом (возвраты за питание)
--        и добавляем эти суммы на авансовый счёт
-- ШАГ 2: Обрабатываем активности с долгами как обычно
-- ============================================
...
BEGIN
  ...
  -- Если активность имеет положительный баланс (refunds > charges - payments),
  -- добавляем эту сумму на авансовый счёт
  -- ============================================
  FOR v_refund_record IN
    ...
    monthly_refunds AS (
      SELECT
        ...
        mp.payments,
        mp.refunds,
        -- Положительный баланс = payments - charges + refunds (если > 0)
      ...
      WHERE (mp.payments - mc.charges + mp.refunds) > 0  -- Только положительные балансы
    )
    SELECT
      mr.enrollment_id,
      mr.activity_id,
    ...
    ORDER BY mr.positive_balance DESC
  LOOP
    -- Добавляем положительный баланс на авансовый счёт
    v_refund_amount := v_refund_record.positive_balance;
    
    -- Обновляем авансовый баланс
    ...
    v_remaining_advance := v_remaining_advance + v_refund_amount;
  END LOOP;

  -- ============================================
  -- ШАГ 2: Обрабатываем долги (как обычно)
  -- ============================================
...
-- Обновляем триггер (без изменений)
...
COMMENT ON FUNCTION public.distribute_advance_payment(UUID, UUID, DECIMAL, DATE) IS 
